export default function Portfolio() {
  const proyectos = [
    {
      ...
      descripcion:
        "Plataforma para eventos y sistema POS de caja en vivo en tiempo real para barra. Control de stock, cobros por métodos de pago y exportación de reportes.",
    ...
    },
    {
      ...
      descripcion:
        "Portfolio profesional de alto rendimiento visual para fotógrafa. Galería interactiva, diseño responsivo ultra fluido y UX minimalista.",
    ...
    },
    {
      ...
      descripcion:
        "Catálogo web interactivo e e-commerce para venta de repuestos automotores. Filtros rápidos y optimización móvil.",
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-violet-500 selection:text-white">
      ...
      {/* NAVBAR */}
      <nav className="fixed top-0 w-full bg-zinc-950/80 backdrop-blur-md border-b border-zinc-800/80 z-50">
      ...
      <main className="max-w-5xl mx-auto px-6 pt-32 pb-20 flex flex-col gap-24">
        
        {/* HERO SECTION */}
        <section className="flex flex-col items-start gap-6 max-w-3xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-violet-950/50 border border-violet-800/50 text-violet-300 text-xs font-semibold">
          ...
          <div className="flex flex-wrap gap-4 pt-2">
            <a
              href="#proyectos"
              className="bg-violet-600 hover:bg-violet-500 text-white font-bold px-6 py-3 rounded-xl transition-all shadow-lg shadow-violet-900/30 flex items-center gap-2"
            >
              Ver mis proyectos
            ...
            <a
              href="#contacto"
              className="bg-zinc-900 hover:bg-zinc-800 text-zinc-300 border border-zinc-800 font-bold px-6 py-3 rounded-xl transition-all"
            >
              Contactar
            </a>
          </div>
        ...
        <section id="proyectos" className="flex flex-col gap-8">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-zinc-400 text-sm">Aplicaciones en producción e implementaciones reales.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {proyectos.map((p, idx) => (
              <div
                ...
                } rounded-2xl p-6 flex flex-col justify-between gap-6 hover:border-violet-500/80 transition-all group`}
              >
                <div className="flex flex-col gap-3">
                  <div className="flex justify-between items-start">
                    ...
                    {p.destacado && (
                      <span className="text-[10px] font-black uppercase bg-violet-600/30 text-violet-300 border border-violet-500/40 px-2 py-0.5 rounded-md">
                        Destacado
                      </span>
                    )}
                  </div>
                ...
                <div className="flex flex-col gap-4">
                  <div className="flex flex-wrap gap-1.5">
                    {p.tags.map((t, i) => (
                      <span key={i} className="text-xs bg-zinc-950 text-zinc-400 border border-zinc-800 px-2.5 py-1 rounded-lg">
                        {t}
                      </span>
                    ))}
                  </div>
                  ...
                  <a
                    ...
                    className="inline-flex items-center gap-2 text-sm font-bold text-violet-400 hover:text-violet-300 transition-colors pt-2"
                  >
                    Visitar Proyecto
        ...
        <section id="seguridad" className="bg-gradient-to-br from-violet-950/30 via-zinc-900 to-zinc-950 border border-violet-800/40 rounded-3xl p-8 sm:p-10 flex flex-col gap-6">
          <div className="inline-flex items-center gap-2 text-violet-400 font-bold text-xs uppercase tracking-widest">
        ...
        <section className="flex flex-col gap-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-zinc-900/40 border border-zinc-800 p-6 rounded-2xl flex flex-col gap-4">
              <h3 className="text-lg font-bold text-violet-400">Core Frontend</h3>
              <ul className="flex flex-wrap gap-2">
                {stack.Frontend.map((s, i) => (
                  <li key={i} className="text-xs bg-zinc-900 border border-zinc-700 text-zinc-200 px-3 py-1.5 rounded-lg">
                    {s}
                  </li>
                ))}
              </ul>
            ...
            <div className="bg-zinc-900/40 border border-zinc-800 p-6 rounded-2xl flex flex-col gap-4">
              <h3 className="text-lg font-bold text-violet-400">Seguridad & AppSec</h3>
              <ul className="flex flex-wrap gap-2">
            ...
            <div className="bg-zinc-900/40 border border-zinc-800 p-6 rounded-2xl flex flex-col gap-4">
              <h3 className="text-lg font-bold text-violet-400">Testing & Herramientas</h3>
              <ul className="flex flex-wrap gap-2">
        ...
        {/* FOOTER / CONTACTO */}
        <footer id="contacto" className="border-t border-zinc-800 pt-12 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex flex-col gap-1 text-center md:text-left">
            <span className="text-lg font-bold text-white">¿Conectamos?</span>
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-zinc-200 text-xs font-bold px-4 py-2.5 rounded-xl transition-all"
            >
              LinkedIn
            </a>
            <a
